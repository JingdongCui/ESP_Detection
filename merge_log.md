/plan 现在给 merge_project 做调试适配。先检查现有的编码器 电机 光电传感器的api.然后要专门在一个文件里面调整调试开关，使用宏定义来调，调整各个硬件的接口，所有传感器的极性正反，每个电机的正转与反转。和一些编码器的设置。然后加一些变量，在ui上面能够控制一些调试参数，能够开关电机控制，调整每个电机的速度，调整安全延时。模拟分类各类包裹的按钮，按下之后 电机模拟识别到了某类包裹，然后做对应的电机控制。对于以太网调试链路，在ui上加一个开关功能，开的时候接收上位机的模拟传感器和识别 关闭的时候使用板子上的硬件。ui上要加
