...
void HostController::setInferenceServiceUrl(const QString &url)
{
    ...
    if (trimmed.isEmpty() || m_inferenceServiceUrl == trimmed) {
        return;
    }
...
{
    QNetworkRequest request(QUrl(m_inferenceServiceUrl + QStringLiteral("/health")));
    QNetworkReply *reply = m_network.get(request);
    connect(reply, &QNetworkReply::finished, this, [this, reply]() {
        const bool ok = reply->error() == QNetworkReply::NoError;
        m_inferenceStatus = ok
            ? QStringLiteral("推理服务在线：%1").arg(QString::fromUtf8(reply->readAll()))
            : QStringLiteral("推理服务不可用：%1").arg(reply->errorString());
        reply->deleteLater();
