...
- `tools/build_competition_report_docx.py` 支持 `输入.md 输出.docx` 两个参数；当前报告 Word 全文使用宋体和黑色字，生成后须按 Documents skill 渲染逐页检查。
...
- 2026-07-26 Host 展示模式：`esp32_host_no_inference` 未连接板子时，`HostController` 输出固定指标、趋势、检测历史、统计与日志，并通过仅展示用的 `displayConnected` 显示设备在线/链路正常；真实 `connected` 保持为 false，仍作为控制命令安全门控。连接后自动恢复真实数据。`ReservePage.qml` 的系统维护页包含纯本地云端大模型回复演示，不发起云端请求，动画为 2 秒转圈、20 字/秒逐字显示、完成后停留 10 秒循环。
