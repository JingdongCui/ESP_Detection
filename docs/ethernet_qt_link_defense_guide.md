...
同一包裹连续被模型命中时不会重复发送。当前代码允许保留 5 个 miss 帧，并在第 6 个连续 miss 时清除上一次命中状态；此后下一次有效命中才会再发一张。换句话说，常量 `VISION_DISPLAY_MISS_KEEP_COUNT=5` 表示“保留 5 帧”，不是“第 5 个 miss 当场清零”。因此：
