...
在部署到 ESP32-P4 时，模型需要经过轻量化和量化处理。项目中采用 MSE equalization 量化策略，参数设置为 `(10, 0.1)`，未使用 TQT，也未启用 bias correction。该策略重点降低模型压缩后对置信度和分类准确率的影响，使模型能够在嵌入式端保持实时推理能力，同时维持较高识别准确率。结合两级检测流程，系统将第二阶段 Logo 检测限制在面单区域内，进一步减少无效搜索范围，提高了边缘端推理效率。
