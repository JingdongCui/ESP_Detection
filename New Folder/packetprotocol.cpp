                        ...
                        ImageResultV2 *result, QString *error)
{
    ...
    for (quint16 i = 0; i < boxCount; ++i) {
        ...
        box.stage = b[0];
        box.category = b[1];
        box.scoreX1000 = qMin<quint16>(1000, qFromLittleEndian<quint16>(b + 2));
        ...
        if (box.x1 > box.x2 || box.y1 > box.y2 || box.x2 >= sourceWidth || box.y2 >= sourceHeight) {
            return fail(QStringLiteral("V2 图像检测框越界"));
        }
        boxes.append(box);
    }

    const QByteArray jpeg = payload.mid(kImageMetaV2Size + boxCount * kImageBoxV2Size, jpegLen);
    if (jpeg.size() != int(jpegLen) || !jpeg.startsWith("\xff\xd8")) {
