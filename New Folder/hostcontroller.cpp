...
void HostController::clearFrameHistory()
{
    ...
    m_latestFrameInfo.clear();
    m_latestCategoryLabel = QStringLiteral("--");
...
void HostController::onNetworkImageResultReady(quint32 frameId, quint16 width, quint16 height,
                                               ...
                                               const QVariantList &boxes, const QByteArray &jpeg)
{
    ...
    if (!saveLatestPreviewImage(frameId, width, height, HostProtocol::kPixelJpeg, jpeg)) {
        appendLog(QStringLiteral("V2 JPEG 解码失败 frame=%1").arg(frameId));
        return;
    }
    ...
    m_latestFrameInfo = QStringLiteral("帧#%1  %2  %3%  %4 个框")
                            .arg(frameId)
                            .arg(m_latestCategoryLabel)
                            .arg(m_latestCategoryConfidence)
                            .arg(detections.size());
    addImageHistoryRecord(frameId, width, height, classId, confidencePct, inferTimeMs,
                          QStringLiteral("V2 JPEG"), m_latestImageUrl, detections);
    emit detectionChanged();
    emit statsChanged();
    emit imageChanged();
    emit dashboardChanged();
...
void HostController::handleImage(const HostProtocol::PacketHeader &header, const QByteArray &payload)
{
    ...
    onNetworkImageResultReady(result.frameId, result.width, result.height,
...
QVariantList HostController::makeImageDetections(quint16 width, quint16 height, const QVariantList &boxes) const
{
    ...
    for (int drawStage = 0; drawStage <= 1; ++drawStage) {
        for (const QVariant &value : boxes) {
            ...
            const int stage = box.value(QStringLiteral("stage")).toInt();
            if (stage != drawStage) {
                continue;
            }
            const int category = box.value(QStringLiteral("category")).toInt();
            const int scoreX1000 = qBound(0, box.value(QStringLiteral("scoreX1000")).toInt(), 1000);
            const int x1 = box.value(QStringLiteral("x1")).toInt();
            const int y1 = box.value(QStringLiteral("y1")).toInt();
            const int x2 = box.value(QStringLiteral("x2")).toInt();
            const int y2 = box.value(QStringLiteral("y2")).toInt();

            QVariantMap detection;
            detection.insert(QStringLiteral("stage"), stage);
            detection.insert(QStringLiteral("category"), category);
            detection.insert(QStringLiteral("confidence"), scoreX1000 / 1000.0);
            detection.insert(QStringLiteral("x"), double(x1) / width);
            detection.insert(QStringLiteral("y"), double(y1) / height);
            detection.insert(QStringLiteral("w"), double(x2 - x1 + 1) / width);
            detection.insert(QStringLiteral("h"), double(y2 - y1 + 1) / height);
            detection.insert(QStringLiteral("label"), stage == 0 ? QStringLiteral("面单") : categoryLabelFromId(category));
            detection.insert(QStringLiteral("color"), stage == 0 ? QStringLiteral("#00ff00")
                                                                   : category == 1 ? QStringLiteral("#ffff00")
                                                                   : category == 2 ? QStringLiteral("#0066ff")
