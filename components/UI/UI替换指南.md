...
> `sdk/` 是手写并长期维护的，**不要用 AnyUI 导出覆盖**。项目不使用桌面模拟器，
> AnyUI 导出的 `simulator/` 内容不要复制到工程，固件也不编译任何模拟器占位代码。
> 因此本指南聚焦于：替换 `generated/` 后，如何让它与不变的 `sdk/` 重新对齐、
> 并通过 ESP-IDF 编译。
...
**不要覆盖** `components/UI/sdk/`。AnyUI 导出的 `simulator/` 目录无需复制。
...
> CMakeLists 中现有的 `sdk/evt.c`、`sdk/scr_list.c`、`sdk/ui.c` 行**保持不动**，
> 它们不随 `generated/` 替换而变。不要加入 `simulator/dummy.c`；项目固件不依赖
> 模拟器代码。`REQUIRES lvgl` 也不要改。
...
`sdk/` 其余文件正常情况下无需改动。`sdk/cmd.c` 不在固件 `SRCS` 中，
因为当前界面没有命令通道调用；不要为它重新引入 `simulator/dummy.c` 占位实现。
