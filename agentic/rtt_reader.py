...
def _kill_existing_readers(platform=None):
    ...
    # Unix fallback: pgrep (safe, skips self by PID comparison)
    # Windows: skip global process scan — it's too dangerous (can kill self)
    if platform != 'windows':
        try:
            result = subprocess.run(
...
def main():
    ...
    if _cfg_path and Path(_cfg_path).exists():
        with open(_cfg_path) as _f:
            _platform = json.load(_f).get('platform', '')
    if not _platform:
