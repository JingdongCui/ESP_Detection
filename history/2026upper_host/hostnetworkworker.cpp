...
void HostNetworkWorker::requestInference(quint32 frameSeq, quint16 width, quint16 height, quint16 pixelFormat, const QByteArray &imagePayload)
{
    ...
    url.setQuery(query);
    if (!url.isValid()) {
        emit inferenceStatusChanged(QStringLiteral("推理服务地址无效"), true);
        return;
    }
    ...
    if (updateUi) {
        emit inferenceStatusChanged(QStringLiteral("正在推理帧 #%1").arg(frameSeq), false);
    }

    QNetworkReply *reply = m_network.post(request, imagePayload);
    const qint64 postStartMs = timer.elapsed();
    connect(reply, &QNetworkReply::finished, this, [this, reply, frameSeq, updateUi, timer, postStartMs]() mutable {
        --m_pendingInferenceRequests;
        if (reply->error() != QNetworkReply::NoError) {
            const QString status = QStringLiteral("帧 #%1 JPEG 推理失败：%2").arg(frameSeq).arg(reply->errorString());
            reply->deleteLater();
            emit inferenceStatusChanged(status, true);
            return;
        }
        ...
        if (!doc.isObject()) {
            reply->deleteLater();
            emit inferenceStatusChanged(QStringLiteral("帧 #%1 推理结果 JSON 无效").arg(frameSeq), true);
            return;
        }
        ...
        QString logLine;
        if (updateUi) {
            logLine = QStringLiteral("帧 #%1 主机链路：post=%2ms http=%3ms infer=%4ms decode=%5ms")
                          .arg(frameSeq)
                          .arg(postStartMs)
            ...
            emit inferenceStatusChanged(QStringLiteral("帧 #%1 推理完成").arg(frameSeq), false);
        }
        emit inferenceFrameReady(frame, updateUi, logLine);
        reply->deleteLater();
    });
}
