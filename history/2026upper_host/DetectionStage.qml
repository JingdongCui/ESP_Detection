...
PremiumPanel {
    ...
    Canvas {
        ...
        Connections {
            target: root
            function onFrameInfoChanged() { demoCanvas.requestPaint() }
            function onThemeChanged() { demoCanvas.requestPaint() }
        }
        onPaint: {
            ...
            ctx.fillText(root.frameInfo.length > 0 ? "历史帧无图像文件" : "选择右侧历史帧回放", width / 2, cardY + cardH - 42)
        }
    }
    ...
    Rectangle {
        ...
        Text {
            ...
            text: root.frameInfo.length > 0 ? root.frameInfo + "    检测目标 " + root.targetCount + " 个" : "未选择回放帧"
            color: theme.text
            font.pixelSize: 14
        ...
        Text {
            ...
            anchors.verticalCenter: parent.verticalCenter
            text: "危险阈值 < " + host.dangerThreshold + "%"
            color: theme.danger
            font.pixelSize: 13
            font.weight: Font.Black
        }
