...
图：系统整体流程图，展示包裹进入、视觉识别、分拣执行、板端显示和上位机监控之间的关系。

从实现语义看，需求分析阶段确定三类快递包裹识别与分流目标；机械与电源阶段完成三段传送带、摄像头支架、电机驱动和低压供电搭建；视觉阶段完成图像采集、面单定位、Logo 分类、量化部署和阈值调试；控制阶段把视觉类别转换为分拣状态机输入，并根据传感器事件、电机速度和超时参数执行分流；通信阶段把遥测、识别图片和控制命令接入上位机，最后通过联调测试优化识别率、分拣速度和显示稳定性。
...
系统层级关系可以用文字描述为：包裹首先经过光电传感器和摄像头形成位置、图像两类输入；ESP32-P4 接收输入后完成图像预处理、模型推理、分拣决策、LCD 显示和以太网通信；执行层根据板端输出的 PWM/方向信号驱动三段传送带完成分流；上位机通过 TCP 5000 接收遥测并下发控制，通过 TCP 5001 接收带框 JPEG 图片，用于现场观察和记录。
...
供电和控制信号的语义为：外部电源先进入降压模块，12V 支路为电机驱动板和直流减速电机供电，3.3V/5V 支路为 ESP32-P4、传感器和低压外设供电。ESP32-P4 通过 GPIO 输出 PWM 与方向控制信号到电机驱动板，电机驱动板再输出大电流驱动传送带电机；光电传感器以 GPIO 输入形式把包裹到位状态反馈给 ESP32-P4，分拣调度器据此更新包裹状态。
...
视觉识别的处理语义为：摄像头输出 RGB888 原图后，推理任务先把原图交给面单检测模型；如果没有检测到面单，则该帧按无目标处理并等待下一帧；如果检测到面单，则选择最高分面单框，从原图裁剪 ROI 子图，再把 ROI 交给 Logo 模型识别极兔、韵达或中通。Logo 结果通过阈值判断后，一方面生成 LCD 画框结果，另一方面把类别和置信度提交给分拣调度，并在新包裹识别成功时触发一次上位机快照。
...
分拣调度的处理语义为：S1 传感器触发后创建包裹对象并打开视觉窗口；视觉模块在窗口内多次提交分类结果，控制层按置信度投票得到最终类别；包裹运行到 S2/S4 等关键位置时，调度器根据包裹类别、当前传送带占用状态、交接延时和超时参数决定电机方向与速度；包裹完成分流后释放轨迹记录，同时通过上位机链路更新记录和统计。
...
上位机数据处理链路可以描述为：`HostNetworkWorker` 在线程中监听 control 和 image 两个 TCP 端口；收到字节流后先由 `PacketProtocol` 校验 40 字节包头并拆出 payload；metrics JSON 进入 `HostController` 后更新 CPU、内存、PSRAM、图像队列和运行时长等属性；JPEG 图片经解码和颜色修正后保存为本地文件并推送到 QML 界面；设备控制页产生的时间同步或电机速度命令则封装成协议包，通过 control 链路发回 ESP32-P4。
