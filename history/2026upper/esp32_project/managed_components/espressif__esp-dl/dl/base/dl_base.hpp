    ...
    if (args.input_height > 4 * args.dilation_h * args.filter_height) {
        if (runtime_mode == RUNTIME_MODE_MULTI_CORE ||
            ...
            if (args.mac_shift == INT_MIN) {
                size_t tie_factor_size =
                    dl::tool::get_aligned_size(args.output_channel * sizeof(feature_t));
                if (args.tie_filter_channel_factor) {
                    m_args[1].tie_filter_channel_factor =
                        (feature_t *)tool::calloc_aligned(tie_factor_size, 1, MALLOC_CAP_DEFAULT);
                    if (m_args[1].tie_filter_channel_factor) {
                        std::memcpy(m_args[1].tie_filter_channel_factor,
                                    args.tie_filter_channel_factor,
                                    tie_factor_size);
                    }
                }
                if (args.filter_channel_factor) {
                    size_t factor_size =
                        dl::tool::get_aligned_size(args.output_channel * sizeof(int16_t));
                    m_args[1].filter_channel_factor =
                        (int16_t *)tool::calloc_aligned(factor_size, 1, MALLOC_CAP_DEFAULT);
                    if (m_args[1].filter_channel_factor) {
                        std::memcpy(m_args[1].filter_channel_factor,
            ...
            if (args.debug_value) {
                m_args[1].debug_value = tool::calloc_aligned(16, 1, MALLOC_CAP_DEFAULT);
                if (m_args[1].debug_value) {
                    std::memcpy(m_args[1].debug_value, args.debug_value, 16);
                }
            }
